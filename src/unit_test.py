...
def test_function_preserving_r2widerr(model, thresh, function_preserving=True, data_channels=1, verbose=False, 
                                      deep=False, spatial_dim=32, net_morph=False):
    # Count params before widening
    params_before = sum([np.prod(p.size()) for p in model.parameters()])
    l1_norm_before = parameter_magnitude(model)
...
class _Baby_Inception(nn.Module):
    ...
    def __init__(self, test_concat=False, add_batch_norm=True):
        ...
        self.linear1 = nn.Linear(60*8*8, 10)
        self.linear2 = nn.Linear(10, 10)
...
if __name__ == "__main__":
    ...
    # volume. So if we widen convolutional layers by a factor 4 then linear by a factor 2, there's not enough 'space' 
    # over the residual connection (trying to add a residual connection from a volume widened by a factor 4 to a volume 
    # widened by a factor 2 doesn't work) 
    if verbose:
        print("\n"*4)
