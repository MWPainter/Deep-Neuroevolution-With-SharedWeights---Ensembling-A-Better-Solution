...
def _update_op(model, optimizer, minibatch, iter, args):
    ...
    if iter in args.widen_times or iter in args.deepen_times:
        if iter in args.widen_times:
            ...
            model = cudafy(net2net_widen_network_(model, new_channels=2, new_hidden_nodes=0,
                                                  multiplicative_widen=True))
        elif iter in args.deepen_times:
            model = net2net_make_deeper_network_(model, args.get_deepen_block(args.function_preserving), batch=xs)
            model = cudafy(net2net_make_deeper_network_(model, args.get_deepen_block(args.function_preserving), batch=xs))
...
def _mnist_test(args, model=None, widen_times=[], deepen_times=[], identity_init_network=False,
...
def _cifar_test(args, model=None, widen_times=[], deepen_times=[], identity_init_network=False,
                function_preserving=False):
    """
    Train a mnist resnet, widening and deepening at some points
    ...
    # Make the model, and run the training loop
    if model is None:
        model = Cifar_Resnet(identity_initialize=False, add_residual=False)
    train_loop(model, train_loader, val_loader, _make_optimizer_fn, _load_fn, _checkpoint_fn, _update_op,
               _validation_loss, args)
...
def mnist_deepen_test(args):
    ...
    args.get_deepen_block = lambda identity: Net2Net_ResBlock_identity(input_channels=32,
                                                                       intermediate_channels=[32, 32, 32],
                                                                       output_channels=32, identity_initialize=identity,
                                                                       input_spatial_shape=(4, 4))

    # identity initialize loop
    args.shard = "Net2DeeperNet"
...
def cifar_deepen_test(args):
    ...
    # Create a resblock that can be used for deepening
    args.get_deepen_block = lambda identity: Net2Net_ResBlock_identity(input_channels=64,
