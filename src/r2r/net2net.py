...
def net_2_wider_net_(prev_layers, next_layers, volume_shape, batch_norm, extra_channels=0, init_type="He", function_preserving=True):
    """
    ...
    _net_2_wider_net_(prev_layers, next_layers, volume_shape, batch_norm, extra_channels, init_type, function_preserving)





def _net_2_wider_net_(prev_layers, next_layers, volume_shape, batch_norm, extra_channels, init_type, function_preserving):
    """  
    ...
    for (layer_index, prev_layer) in enumerate(prev_layers):
        ...
        for extra_channel_index in range(out_channels, out_channels + extra_channels):
    ...
    for (layer_index, prev_layer) in enumerate(prev_layers):
        _net2net_widen_output_channels_(prev_layer, extra_channels, init_type, extra_channels_mappings[layer_index])
    
    # Widen batch norm appropriately 
    if batch_norm:
        _net2net_extend_bn_(batch_norm, extra_channels)
    ...
    for (layer_index, next_layer) in enumerate(next_layers):
        _net2net_widen_input_channels_(next_layer, extra_channels, init_type, volume_slices_indxs, extra_channels_mappings[layer_index],)



def _net2net_extend_bn_(bn, new_channels_per_slice, volume_slice_indxs, scaled, extra_channels_mappings):
    """
    ...
    for i in range(1, len(volume_slice_indxs)):
        ...
        extra_channels_mapping = extra_channels_mappings[i]
...
def _net2net_widen_output_channels_(prev_layer, extra_channels, init_type, extra_channels_mapping, noise):
    """
    ...
    if type(prev_layer) is nn.Conv2d:
        ...
        new_kernel.resize_(out_channels + extra_channels, in_channels, width, height)

        new_bias = prev_bias.clone()
        new_bias.resize_(out_channels + extra_channels)

        new_kernel.narrow(0, 0, out_channels).copy_(prev_kernel)
        new_bias.narrow(0, 0, out_channels).copy_(prev_bias)

        for index in range(out_channels, out_channels + extra_channels):
        ...
        new_matrix.resize_(n_out + extra_channels, n_in)

        new_bias = prev_bias.clone()
        new_bias.resize_(n_in + extra_channels)

        new_matrix.narrow(0, 0, n_out).copy_(prev_matrix)
        new_bias.narrow(0, 0, n_out).copy_(prev_bias)

        for index in range(n_out, n_out + extra_channels):
        ...
        _assign_weights_and_bias_to_linear_(prev_layer, prev_matrix.numpy(), prev_bias.numpy())
...
def _net2net_widen_input_channels_(next_layer, extra_channels, volume_slice_indxs,
                                   input_linear, extra_params_per_input, extra_channels_mapping):
    """
    ...
    if type(next_layer) is nn.Conv2d:
        ...
        for i in range(1, len(volume_slice_indxs)):
            ...
            kernel_part.resize_(out_channels, in_channels+extra_channels, width, height)
        ...
        for i in range(1, len(volume_slice_indxs)):
            ...
            matrix_part.resize_(n_out, n_in + extra_channels)

            matrix_part = _net2net_extend_filter_input_channels(original_matrix, matrix_part,
                                                                n_in, extra_channels, extra_channels_mapping)
