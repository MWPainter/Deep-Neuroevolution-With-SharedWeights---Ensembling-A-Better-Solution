...
class MnistDataset(Dataset):
    def __init__(self, batch_size=32, binary=False):
    ...
    def next_batch(self, batch_size=None):
        ...
        images = np.reshape(xs, (-1, 28, 28, 1))
        images = np.reshape(images, (-1, 28, 28, 1))
        images = np.pad(images, ((0,0),(2,2),(2,2),(0,0)), 'edge') # pads images to be 32x32 vs 28x28
        if self.binary:
            return np.rint(images), labels
        else:
